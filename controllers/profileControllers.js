...
angular.module('esn')
    .controller('profileCtrl', function ($scope, $http, $routeParams, $Auth, fileUpload) {
        ...
        var getUserInfo = function () {
            ...
            return $http.get('http://imadevelopers.com/esn/users/getByUsername/' + $routeParams.userID + '.json', {withCredentials: true})
                .success(function (data) {
                    console.log(data);
                    if (data.users.length != 0) {
                        $scope.userInfoLoading = false;
                        $scope.Auth.user = data.users;
                        $http.get('http://imadevelopers.com/esn/users/areConnected/' + $scope.Auth.user.User.id + '.json', {withCredentials: true})
                            .success(function (data) {
                        ...
                        $http.get('http://imadevelopers.com/esn/users/getJoinedGroups/' + $routeParams.userID + '.json', {withCredentials: true})
                            .success(function (data) {
                                if (data.result.success) {
        ...
        $scope._editTagLine = function () {
            if ($scope.profile.editTagLine) {
                ...
                $_user.User.tagline = $scope.profile.user.tagline;
                var $url = 'http://imadevelopers.com/esn/users/edit/' + $Auth.getUser('id') + '.json';
                $http.post($url, $_user, {withCredentials: true})
                    .success(function (data) {
                        console.log(data);
                    })
                    .error(function (error) {
        ...
        $scope.profile_photo_uploading = false;
        $scope.profile_cover_uploading = false;
        ...
        $scope.uploadfile = function () {
            ...
            if ($scope.upload.file == 'cover') {
                ...
                uploadUrl = 'http://imadevelopers.com/esn/users/addCoverPhoto.json';
                $scope.profile_photo_uploading = false;
                $scope.profile_cover_uploading = true;
            } else if ($scope.upload.file == 'profile') {
                file = $scope.profileFile;
                uploadUrl = 'http://imadevelopers.com/esn/users/addProfilePhoto.json';
                $scope.profile_photo_uploading = true;
                $scope.profile_cover_uploading = false;
            }
        ...
        $scope.connectUser = function () {
            ...
            $http.post('http://imadevelopers.com/esn/users/addContact.json', $formData, {withCredentials: true})
                .success(function (data) {
                    if (data.result.success) {
                        $scope.connectedUser = true;
                    }
        ...
        $scope.disconnectUser = function () {
            $http.delete('http://imadevelopers.com/esn/users/deleteContact/' + $routeParams.userID + '.json', {withCredentials: true})
                .success(function (data) {
                    console.log(data);
                    if (data.result.success) {
                        $scope.connectedUser = false;
    ...
    controller('profileMenuCtrl', function ($scope) {
        ...
        $scope.deleteInterest = function (id, $index) {
            $http.delete('http://imadevelopers.com/esn/users/deleteInterest/' + id + '.json', {withCredentials: true})
                .success(function (data) {
                    if (data.result.success) {
        ...
        $scope.addInterest = function () {
            ...
            if (!angular.isUndefined($scope.data.selectedInterest)) {
                ...
                $http.post('http://imadevelopers.com/esn/users/addInterest.json', $data, {withCredentials: true})
                    .success(function (data) {
        ...
        $scope.addEducation = function () {
            ...
            $http.post('http://imadevelopers.com/esn/users_educations/add.json', $formData, {withCredentials: true})
                .success(function (data) {
                    console.log(data);
                    if (data.result.success) {
        ...
        $scope.deleteEducation = function ($id, $index) {
            $http.delete('http://imadevelopers.com/esn/users_educations/delete/' + $id + '.json', {withCredentials: true})
                .success(function (data) {
                    if (data.result.success) {
        ...
        var updatePosts = function () {
            ...
            $http.post('http://imadevelopers.com/esn/posts/getForUserProfile/' + $scope.profile.user.id + '.json', {withCredentials: true})
                .success(function (data) {
                    if (data.result.success) {
                        $scope.postsLoading = false;
